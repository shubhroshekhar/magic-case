...
export default function runErrorTests() {
  describe('Error Handling', () => {
    test('BaseCase type validation', () => {
      expect(() => new BaseCase(123)).toThrow(
        'BaseCase expects a string or another BaseCase instance'
      );
      expect(() => new BaseCase(null)).toThrow(
        'BaseCase expects a string or another BaseCase instance'
      );
      expect(() => new BaseCase(undefined)).toThrow(
        'BaseCase expects a string or another BaseCase instance'
      );
    });
    ...
    test('PathCase validation', () => {
      expect(() => new PathCase('Invalid/Path')).toThrow(
        'Invalid PathCase string'
      );
      expect(() => new PathCase('API/Users')).toThrow(
        'Invalid PathCase string'
      );
    ...
    test('SlashTitleCase validation', () => {
      ...
      expect(() => new SlashTitleCase('/hello/world')).toThrow(
        'Invalid SlashTitleCase: cannot start or end with slash'
      );
      expect(() => new SlashTitleCase('hello/world/')).toThrow(
        'Invalid SlashTitleCase: cannot start or end with slash'
      );
      expect(() => new SlashTitleCase('hello//world')).toThrow(
        'Invalid SlashTitleCase: consecutive slashes not allowed'
      );
      expect(() => new SlashTitleCase('hello/world')).not.toThrow();
    });

    test('Function parameter validation', async () => {
      const { toCamelCase, toHttpHeaderCase, toPathCase, toSlashTitleCase } =
      ...
      expect(() => toPathCase('Invalid/Path')).toThrow(
        'Invalid PathCase string'
      );
